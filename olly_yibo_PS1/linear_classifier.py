...
"""
from matplotlib.rcsetup import _validate_linestyle
import torch
import random
import statistics
...
def sample_batch(
    ...
    #########################################################################
    rand_int = torch.randint(0, num_train, (batch_size,))
    X_batch = X[rand_int,:]
    y_batch = y[rand_int,]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def test_one_param_set(
    ...
    #X_batch, y_batch = sample_batch(X_train,y_train,X_train.shape[0],X_train.shape[0]/num_iters)
    #W, loss_histroy = train_linear_classifier()
    W, loss_hist = train_linear_classifier(cls.loss, None, X_train, y_train, lr, reg, num_iters)
    train_y = predict_linear_classifier(W, X_train)
    train_acc = 100.0 * (y_train == train_y).double().mean().item()
    val_y = predict_linear_classifier(W, X_val)
    val_acc = 100.0 * (val_y == y_val).double().mean().item()
    cls.W = W
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(y.shape[0]):
      loss_aw = torch.divide(torch.exp(label[i]),torch.sum(torch.exp(label[i])))
      loss -= torch.log(loss_aw[y[i]])

      for j in range(W.shape[1]):
        if j == y[i]:
          dW[:,j] += (loss_aw[y[i]]-1)*X[i] + 2*reg*W[:,j]
        else:
          dW[:,j] += (loss_aw[j])*X[i] + 2*reg*W[:,j]
...
def softmax_loss_vectorized(
    ...
    label = torch.mm(X,W)
    label -= torch.max(label, dim=1, keepdim=True).values
    loss_aw = torch.exp(label) / torch.sum(torch.exp(label),dim=1,keepdim=True)
    loss = -(torch.sum(torch.log(loss_aw[range(X.shape[0]),y]))/y.shape[0]) + reg*torch.sum(W.pow(2))

    dW = loss_aw
    dW[range(X.shape[0]),y] -= 1
    dW = torch.mm(dW.t(), X).t()
    dW /= y.shape[0]
    dW += 2*reg*W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [37e-4, 5e-3, 77e-4]
    regularization_strengths = [56e-4, 333e-5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
