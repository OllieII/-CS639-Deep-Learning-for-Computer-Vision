...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # print(N, F, outH, outW)
        out = torch.zeros((N, F, outH, outW), dtype=x.dtype, device=x.device)
        for a in range(0, N):
          for c in range(0, F):
            filter = w[c].view(-1,1)
            for i in range(0, outH):
              for j in range(0, outW):
                idxI = i * stride
                idxJ = j * stride
                segment = x_padded[a, :, idxI:idxI+HH, idxJ:idxJ+WW].reshape(1, -1)
                #print(HH)
                #print(filter.shape)
                #print(segment.shape)
                #print(torch.sum(torch.matmul(segment, w[c]))+b[c])
                out[a,c,i,j] = torch.mm(segment,filter)+b[c]
        #print(out)
    ...
    def backward(dout, cache):
        ...
        dx_padded = torch.zeros(x_padded.shape, dtype=x.dtype, device=x.device)
        for n in range(0,N):
          for f in range(0,F):
            dy = dout[n,f,:,:]
            db[f] += torch.sum(dy)
            for c in range(0, C):
              for i in range(0, Ho):
                for j in range(0, Wo):
                  idxI = i * stride
                  idxJ = j * stride
                  segment = x_padded[n, c, idxI:idxI+HH, idxJ:idxJ+WW]
                  dw[f,c,:,:] += segment * dy[i,j]
                  dx_padded[n, c, idxI:idxI+HH, idxJ:idxJ+WW] += w[f,c,:,:] * dy[i,j]
    ...
    def forward(x, pool_param):
        ...
        for n in range(0, N):
          for c in range(0, C):
            for i in range(0, HH):
              for j in range(0, WW):
                out[n,c,i,j] = torch.max(x[n,c,i*stride:i*stride+width,j*stride:j*stride+height])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(0, N):
          for c in range(0, C):
            for i in range(0, HH):
              for j in range(0, WW):
                segment = x[n,c,i*stride:i*stride+width,j*stride:j*stride+height]
                max_val = torch.max(segment)
                res = (segment==max_val).int()
                dx[n,c,i*stride:i*stride+width,j*stride:j*stride+height] = dout[n,c,i,j]*res
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params["W2"] = torch.randn([hidH*hidW*num_filters, hidden_dim], dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        scores = X
        scores, cache1 = Conv_ReLU_Pool.forward(scores, W1, b1, conv_param, pool_param)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        h = input_dims[1]
        w = input_dims[2]
        kaiming = True if (weight_scale == "kaiming") else False
        for i in range(self.num_layers):
          W_idx, b_idx = ('W'+str(i+1),'b'+str(i+1))
          gamma, beta = ('gamma'+str(i+1), 'beta'+str(i+1))
          if i == 0:
            if not kaiming:
              self.params[W_idx] = torch.randn([num_filters[i], input_dims[0], 3, 3], dtype=dtype, device=device) * weight_scale
              #print(self.params[W_idx].shape)
            else:
              self.params[W_idx] = kaiming_initializer(num_filters[i], input_dims[0], 3, dtype=dtype, device=device)
            self.params[b_idx] = torch.zeros(num_filters[i], dtype=dtype, device=device)
            # h = 1 + (h - 3) // 2
            # w = 1 + (w - 3) // 2
            if(batchnorm):
              self.params[gamma] = torch.ones(num_filters[i],  dtype=dtype, device=device)
              self.params[beta] = torch.zeros(num_filters[i],  dtype=dtype, device=device)
          elif i == len(num_filters):
            x = len(max_pools)
            h = h // (2 ** x)
            w = w // (2 ** x)
            if not kaiming:
              self.params[W_idx] = torch.randn([h*w*num_filters[i-1], num_classes], dtype=dtype, device=device) * weight_scale
            else:
              self.params[W_idx] = kaiming_initializer(h*w*num_filters[i-1], num_classes, None, dtype=dtype, device=device )
            self.params[b_idx] = torch.zeros(num_classes, dtype=dtype, device=device)
          else:             
            # if(i in max_pools):
            #   self.params[W_idx] = torch.randn([num_filters[i], num_filters[i-1], 3, 3], dtype=dtype, device=device) * weight_scale
            #   self.params[b_idx] = torch.randn([num_filters[i]], dtype = dtype, device = device)
            #   #h = int((h-2)/2 + 1)
            #   #w = int((w-2)/2 + 1)
            # else:
            if not kaiming:
              ...
              self.params[W_idx] = kaiming_initializer(num_filters[i], num_filters[i-1], 3, dtype=dtype, device=device )
            self.params[b_idx] = torch.zeros(num_filters[i], dtype=dtype, device=device)
              # h = int(1 + (h - 3) // 2)
              # w = int(1 + (w - 3) // 2)
            if(batchnorm):
    ...
    def loss(self, X, y=None):
          ...
          for i in range(self.num_layers-1):
            ...
            if i in self.max_pools:
              scores, cache = Conv_BatchNorm_ReLU_Pool.forward(
                scores, W, b, gamma, beta, conv_param, self.bn_params[i], pool_param)
            else:
              scores, cache = Conv_BatchNorm_ReLU.forward(
                scores, W, b, gamma, beta, conv_param, self.bn_params[i])
            caches.append(cache)
        else:
          for i in range(self.num_layers-1):
            W_idx, b_idx = ('W'+str(i+1),'b'+str(i+1))
            ...
            if i in self.max_pools:
              ...
              scores, cache = Conv_ReLU.forward(
                scores, W, b,conv_param)
            caches.append(cache)
          # scores, cache = FastConv.forward(scores, W, b, conv_param)
          # caches.append(cache)
          # if self.batchnorm:
          #   gamma, beta = ('gamma'+str(i+1), 'beta'+str(i+1))
          #   scores, cache = SpatialBatchNorm.forward(scores, self.params[gamma], self.params[beta], self.bn_params[i])
          #   caches.append(cache)
        ...
        W_idx, b_idx = ('W'+str(self.num_layers),'b'+str(self.num_layers))
        W = self.params[W_idx]
        b = self.params[b_idx]
        #print(scores.shape)
        scores, cache = Linear.forward(scores.contiguous(), W, b)
        caches.append(cache)
        # print(list(map(type,caches)))
        # print(list(map(len,caches)))
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        W_idx, b_idx = ('W'+str(self.num_layers),'b'+str(self.num_layers))
        grads[W_idx] = dw + 2 * self.reg * self.params[W_idx]
        grads[b_idx] = db
        loss += self.reg * (torch.sum(self.params[W_idx]**2))
        # for i in range(self.num_layers-2, -1, -1):
        #   W_idx, b_idx = ('W'+str(i+1),'b'+str(i+1))
        #   W = self.params[W_idx]
        #   b = self.params[b_idx]
        #   if i in self.max_pools:
        #     dx = FastMaxPool.backward(dx, caches.pop())
        #   dx = ReLU.backward(dx, caches.pop())
        #   if self.batchnorm:
        #     gamma, beta = ('gamma'+str(i+1), 'beta'+str(i+1))
        #     dx, dgamma, dbeta = SpatialBatchNorm.backward(dx, caches.pop())
        #     grads[gamma] = dgamma
        #     grads[beta] = dbeta
        if self.batchnorm:
          for i in range(self.num_layers-2, -1, -1):
            ...
            if i in self.max_pools:
              dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dx, caches.pop())
            else:
              dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dx, caches.pop())
            grads[W_idx] = dw + 2 * self.reg * W
            grads[b_idx] = db
            grads[gamma] = dgamma
            grads[beta] = dbeta
          ...
          #dx, dw, db = FastConv.backward(dx, caches.pop())
          # grads[W_idx] = dw + 2 * self.reg * W
          # grads[b_idx] = db
          # loss += self.reg * (torch.sum(W**2))
        #############################################################
...
def find_overfit_parameters():
    ...
    learning_rate = 376e-5  # Experiment with this!
    ###########################################################
    #                       END OF YOUR CODE                  #
    ###########################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data, print_every=500,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        fan_in = Din * K * K
        weight = torch.randn([Din, Dout, K, K], dtype=dtype, device=device) * (gain/fan_in) ** 0.5
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            out, cache = out, (sample_mean, sample_var, gamma, beta, eps, x_hat, x)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            out = gamma*x_hat+beta
            out, cache = out, (running_mean, running_var, gamma, beta, eps, x_hat, x)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        mu, var, gamma, beta, eps, x_hat, x = cache
        N, D = dout.shape
